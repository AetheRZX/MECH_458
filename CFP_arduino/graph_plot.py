...
# 1. Position
axs[0, 0].plot(df['Time_Sec'], df['Target'], 'k--', alpha=0.5, label='Target')
axs[0, 0].plot(df['Time_Sec'], df['M1_Pos'], 'b', label='M1')
axs[0, 0].plot(df['Time_Sec'], df['M2_Pos'], 'r', label='M2')
axs[0, 0].set_title('Position')
axs[0, 0].legend()
axs[0, 0].grid(True)

# 2. Sync Error (TUNE PID HERE)
axs[0, 1].plot(df['Time_Sec'], df['Sync_Error'], 'purple')
...
# 3. RPM
axs[1, 0].plot(df['Time_Sec'], df['M1_RPM'], 'b', label='M1')
axs[1, 0].plot(df['Time_Sec'], df['M2_RPM'], 'r', label='M2')
axs[1, 0].set_title('Speed (RPM)')
axs[1, 0].legend()
axs[1, 0].grid(True)

# 4. PWM
axs[1, 1].plot(df['Time_Sec'], df['PWM1'], 'b', label='M1')
axs[1, 1].plot(df['Time_Sec'], df['PWM2'], 'r', label='M2')
