...
AutoTuner::Loop AutoTuner::loop = AutoTuner::MASTER;
...
bool AutoTuner::chainToSlave = false;
long AutoTuner::chainTargetSlave = 0;
bool AutoTuner::chainApplySlave = false;
...
void AutoTuner::start(long target, bool apply, Loop which) {
    ...
    chainToSlave = (which == SLAVE) ? false : chainToSlave;
    loop = which;
    stepTarget = target == 0 ? TUNE_STEP_TARGET : target;
    if (loop == SLAVE && target == 0) stepTarget = TUNE_STEP_TARGET_M2;
    applyResult = apply;
    resetMetrics();
    MotorControl::setZero();
    if (loop == MASTER) MotorControl::getMasterPID(baseKp, baseKi, baseKd);
    else MotorControl::getSlavePID(baseKp, baseKi, baseKd);
    stage = PRE_SETTLE;
    stageStart = millis();
    logCount = 0;
    Serial.print("TUNE: step start ");
    Serial.print(loop == MASTER ? "M1" : "M2");
    Serial.print(", target=");
...
void AutoTuner::startBoth(long targetMaster, long targetSlave, bool applyMaster, bool applySlave) {
    if (stage != IDLE) {
        Serial.println("TUNE: already running");
        return;
    }
    loop = MASTER;
    chainToSlave = true;
    chainTargetSlave = targetSlave == 0 ? TUNE_STEP_TARGET_M2 : targetSlave;
    chainApplySlave = applySlave;
    start(targetMaster == 0 ? TUNE_STEP_TARGET : targetMaster, applyMaster, MASTER);
}
...
void AutoTuner::resetMetrics() {
    ...
    dwellStart = 0;
    settledAt = 0;
    extremePos = (loop == MASTER) ? MotorControl::getM1Pos() : MotorControl::getM2Pos();
    steadyErr = 0;
}
...
void AutoTuner::tick() {
    ...
    case PRE_SETTLE: {
        ...
        long err = MotorControl::getM1Pos();
        float rpm = MotorControl::getM1RPM();
    ...
    case MEASURE: {
        long ref = stepTarget;
        long pos = (loop == MASTER) ? MotorControl::getM1Pos() : MotorControl::getM2Pos();
        long err = ref - pos;
        if (loop == SLAVE) {
            // ensure reference tracks current target in case host changed it mid-run
            ref = MotorControl::getTargetPos();
            err = ref - pos;
        }
        if (stepTarget >= 0) {
        ...
        float rpm = (loop == MASTER) ? MotorControl::getM1RPM() : MotorControl::getM2RPM();
...
void AutoTuner::evaluate() {
    ...
    Serial.println(loop == MASTER ? "M1" : "M2");
    ...
    if (applyResult) {
        if (loop == MASTER) MotorControl::setMasterPID(kp, ki, kd);
        else MotorControl::setSlavePID(kp, ki, kd);
    ...
    if (chainToSlave && loop == MASTER) {
        // launch slave tune immediately
        chainToSlave = false;
        stage = IDLE; // allow next start
        start(chainTargetSlave, chainApplySlave, SLAVE);
        return;
    }
